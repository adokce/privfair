...
void send_private_inputs(const vector<T>& values, vector<ssl_socket*>& sockets, int nparties)
{
    ...
    // cout << num_inputs << " : Sending inputs";
...
void one_run(vector<T>& values, vector<ssl_socket*>& sockets, int nparties)
{
    ...
    // Run the computation
    int steps = 10000,i;
    int send_times = values.size() / steps;

    for(i = 0; i < send_times; i++)
    {
          auto first = values.begin() + steps*i;
          auto last = values.begin() + steps*i + steps;
          vector<T> to_send(first,last);
          send_private_inputs<T>(to_send, sockets, nparties);
    }
    ...
    //          metrics.push_back(receive_result<T>(sockets, nparties));
    //	      cout << metrics[res] << endl;
   // }
   T eo_class_0 = receive_result<T>(sockets, nparties);	
   T eo_class_1 = receive_result<T>(sockets, nparties);
   T eo_class_2 = receive_result<T>(sockets, nparties);
   T eo_class_3 = receive_result<T>(sockets, nparties);
   T eo_class_4 = receive_result<T>(sockets, nparties);
   T eo_class_5 = receive_result<T>(sockets, nparties);
   T eo_class_6 = receive_result<T>(sockets, nparties);
   T acc_m = receive_result<T>(sockets, nparties);
   T acc_f = receive_result<T>(sockets, nparties);
   T acc = receive_result<T>(sockets, nparties); 

   cout << eo_class_0 << " " << eo_class_1 << " " << eo_class_2 << " " << eo_class_3 << " " << eo_class_4 << " " << eo_class_5 << " " << eo_class_6 << " " << acc_m << " " << acc_f << " " << acc << endl;
}
...
void run(vector<T>& values, vector<ssl_socket*>& sockets, int nparties)
{
    ...
    //       std::cout << values[i] << ' ';
    cout << values.size() << " Size of data before calling one run\n";
...
int main(int argc, char** argv)
{
    ...
    int finish;
    int port_base = 14000;
    string folder = "/home/sikha/MP-SPDZ/data"
    ...
    std::vector<Z2<64>> data;

    std::fstream myfile(folder+to_string(my_client_id)+".txt", std::ios_base::in);
   
    double a;
    while (myfile >> a)
    {
        data.push_back(long(round(a * exp2(16))));
    }
    cout << data.size() <<" :Read data of size \n";
